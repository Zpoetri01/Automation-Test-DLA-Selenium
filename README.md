...
Proyek ini adalah automation testing untuk aplikasi **NewDLA** (SPA berbasis
ExtJS), dibangun dengan **Python** dan **Selenium WebDriver** di atas
framework **Pytest**, serta menerapkan pendekatan **Page Object Model
(POM)** dan **data-driven testing**. Seluruh alur pengujian end-to-end,
dari **Login** hingga **Logout**, dijalankan dalam **satu sesi browser**
tanpa perlu me-refresh halaman.

Pengujian mencakup 20 langkah yang dikelompokkan ke dalam 14 modul, yaitu
Dashboard, Tugas, Masuk, Disposisi Keluar, Progress Surat, Surat Keluar
Eksternal, Nota Dinas Keluar, Topbar, dan Logout. Seluruh data uji terpusat
dalam satu file sehingga mudah dipelihara dan dapat diganti tanpa mengubah
kode.
...
> **Catatan:** langkah-langkah dalam satu modul saling bergantung. Sebagai
> contoh, `test_hapus_draft` memerlukan draft yang dibuat oleh
> `test_simpan_draft_pertama`. Oleh karena itu, setiap modul sebaiknya
> dijalankan secara lengkap dan berurutan.
...
2. **Inheritance bertingkat** — logika yang dipakai banyak modul dinaikkan ke
...
3. **Data-driven** — seluruh data uji terpusat di `data/test_data.json` dan
   dimuat melalui fixture `test_data` di `conftest.py`, sehingga data uji
...
└── FilterableListPage            <- logika filter (Modul 2–5)
    ...
    └── SuratFormBasePage         <- logika form surat (Modul 6–7)
...
maka method tersebut diwarisi dari parent class-nya (lihat peta kelas di
atas).
...
```
test_fixed/
├── conftest.py                       <- driver session-scoped + ringkasan hasil di terminal
...
├── data/
│   └── test_data.json                <- seluruh data uji (keyword, isian form, staf, dll.)
├── pages/
...
│   ├── filterable_list_page.py       <- logika filter bersama Modul 2–5
...
│   ├── surat_form_base_page.py       <- logika form surat bersama Modul 6–7
...
├── tests/
│   └── test_e2e_dla.py               <- satu file berisi alur berurutan seluruh modul
├── tools/
│   ├── scan_locators_v2.py           <- scan locator halaman aktif (membutuhkan Chrome pada port 9222)
│   ├── scan_topbar_sekarang.py       <- scan locator topbar halaman aktif (membutuhkan Chrome pada port 9222)
...
> **Versi yang teruji pada proyek ini:** Python 3.13.2 · Selenium 4.45.0 ·
> Pytest 9.1.1 · pytest-html 4.2.0
>
> **Prasyarat:** Google Chrome terinstal, dan akun Google yang terdaftar
> di NewDLA (untuk SSO).

### 2. Environment Variables (Opsional)

Proyek ini tidak menggunakan file `.env`. Seluruh konfigurasi dibaca dari
environment variable melalui `os.getenv()` di `config/config.py`. Karena
setiap variabel memiliki nilai default, proyek dapat langsung dijalankan
...
| `DLA_ACTION_PACE` | `2` | Jeda antar-aksi (detik) — hanya untuk kebutuhan visual, bukan untuk menunggu elemen |
...
> **Catatan:** variabel hanya berlaku pada sesi terminal tempat variabel
> tersebut disetel. Untuk membuatnya permanen, tuliskan pada profil shell
> (misalnya `$PROFILE` pada PowerShell).

### 3. Login Google SSO (Sekali Saja)

Aplikasi NewDLA menggunakan tombol **"Login with Google"**. Selenium tidak
mengisi email/password Google secara langsung karena diblokir oleh Google.
Solusi yang digunakan adalah menjalankan Chrome dengan profil yang sesi
Google-nya sudah aktif.

Pada mesin ini, profil telah tersedia di `D:\selenium-chrome-profile` —
path tersebut merupakan nilai default di `config.py`, sehingga otomatis
digunakan tanpa perlu menyetel `DLA_CHROME_PROFILE`. Untuk mesin atau
anggota tim baru, buat profilnya terlebih dahulu:

1. Tutup semua jendela Chrome.
2. Jalankan Chrome dengan folder profil khusus:
...
3. Login secara manual menggunakan akun Google yang terdaftar di NewDLA,
   lalu tutup Chrome.
4. Setel environment variable `DLA_CHROME_PROFILE` ke path yang sama (lihat
   tabel di atas), atau ubah nilai default `CHROME_PROFILE_PATH` pada
   `config/config.py`.

> **Catatan:** profil Chrome berisi data sesi pribadi. Pola
> `selenium-chrome-profile/` telah terdaftar di `.gitignore`, sehingga
> folder profil (misalnya di dalam proyek) tidak akan ikut ter-commit.
...
> **Catatan:** apabila hanya sebagian method yang dijalankan, browser
> tetap memerlukan login terlebih dahulu. Sertakan `test_login` pada
> filter `-k`.
...
- Isian form Modul 6 (Kepada, Alamat, Perihal, Jenis Surat, dan lain-lain)
  di bagian `surat_dinas_eksternal`.
...
- Upload berkas menggunakan opsi **Link** (`url_dokumen`), bukan file lokal.

Untuk mengubah data uji, cukup dengan mengubah file JSON tersebut tanpa
perlu mengubah kode.

### 6. Laporan

Setelah pytest selesai dijalankan, buka `reports/report.html`. Laporan ini
bersifat self-contained dan berisi ringkasan PASS/FAIL, detail setiap
langkah, serta screenshot otomatis apabila terdapat test yang gagal.

## Kesimpulan

Secara keseluruhan, proyek ini menerapkan Page Object Model dengan data uji
terpusat dan satu sesi browser untuk seluruh alur pengujian. Ringkasan
poin-poin utamanya adalah sebagai berikut:

| Aspek | Penerapan |
|---|---|
| **Pola desain** | Page Object Model (POM) — setiap modul memiliki kelas halaman (page class) sendiri |
| **Data uji** | Terpusat di `data/test_data.json` — data dapat diganti tanpa mengubah kode |
| **Sesi browser** | `scope="session"` — Chrome dibuka sekali untuk seluruh alur |
| **Laporan** | HTML otomatis (`reports/report.html`) dan screenshot saat test gagal |
| **Fleksibilitas** | Enam environment variable untuk menyesuaikan URL, profil Chrome, timeout, dan lainnya |

Dokumentasi ini ditulis mengikuti kondisi aktual repositori `test_fixed`.
